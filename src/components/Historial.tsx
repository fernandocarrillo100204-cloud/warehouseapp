            ...
            <table className="w-full text-left border-collapse" id="audit-log-table">
              ...
              <tbody className="divide-y divide-[#E2E8F0] dark:divide-[#263449] text-[#172033] dark:text-[#F8FAFC] text-xs sm:text-sm">
                {filteredMovimientos.map((mov) => {
                  ...
                  switch (mov.tipo) {
                    case "entrada":
                      badgeColorClass = "bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800";
                      typeLabel = "Entrada";
                      qtyPrefix = "+";
                      qtyColorClass = "text-[#059669] dark:text-emerald-400 font-bold";
                      break;
                    case "salida":
                      badgeColorClass = "bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800";
                      typeLabel = "Salida";
                      qtyPrefix = "-";
                      qtyColorClass = "text-rose-600 dark:text-rose-400 font-bold";
                      break;
                    case "transferencia":
                      badgeColorClass = "bg-sky-50 dark:bg-sky-950/50 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-800";
                      typeLabel = "Transferencia";
                      qtyPrefix = "⇆";
                      qtyColorClass = "text-sky-600 dark:text-sky-400 font-semibold";
                      break;
                  }

                  return (
                    <tr key={mov.id} className="hover:bg-[#F1F5F9] dark:hover:bg-[#182235]/60 transition-colors">
                      {/* Folio */}
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-[#ECFDF5] dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 text-[#059669] dark:text-emerald-400">
                          {mov.folio || "—"}
                        </span>
                      </td>
                      ...
                      {/* Product details */}
                      <td className="py-2.5 px-3">
                        <div className="font-semibold text-[#172033] dark:text-[#F8FAFC] leading-tight">
                      ...
                      <td className="py-2.5 px-3 text-right">
                        <button
                          ...
                          className="p-1.5 text-[#64748B] dark:text-[#94A3B8] hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-md transition-colors border border-transparent hover:border-rose-200 dark:hover:border-rose-800/80 inline-flex items-center gap-1 text-[11px] font-medium"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-[#111827] border border-[#E2E8F0] dark:border-[#263449] rounded-2xl max-w-md w-full p-5 sm:p-6 shadow-xl space-y-4">
            <div className="flex items-center space-x-3 text-rose-600 dark:text-rose-400">
              <div className="p-2.5 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-800 rounded-xl">
                <AlertTriangle className="h-5 w-5 text-rose-600 dark:text-rose-400" />
              </div>
              <div>
                <h3 className="text-base font-bold text-[#172033] dark:text-[#F8FAFC]">¿Eliminar Movimiento?</h3>
                <p className="text-xs text-[#64748B] dark:text-[#94A3B8]">Esta acción removerá el registro del historial.</p>
              </div>
            </div>
            ...
            <div className="bg-[#F8FAFC] dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-[#263449] rounded-xl p-3.5 space-y-2 text-xs text-[#172033] dark:text-[#F8FAFC]">
              {movToDelete.folio && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-bold text-[#059669] dark:text-emerald-400">{movToDelete.folio}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-[#64748B] dark:text-[#94A3B8]">Producto:</span>
                <span className="font-semibold text-[#172033] dark:text-[#F8FAFC]">{getProductName(movToDelete.sku)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#64748B] dark:text-[#94A3B8]">SKU:</span>
                <span className="font-mono text-[#64748B] dark:text-[#94A3B8]">{movToDelete.sku}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#64748B] dark:text-[#94A3B8]">Tipo / Cantidad:</span>
                <span className="capitalize font-semibold text-[#172033] dark:text-[#F8FAFC]">{movToDelete.tipo} ({movToDelete.cantidad} uds)</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#64748B] dark:text-[#94A3B8]">Almacén:</span>
                <span>{getWarehouseName(movToDelete.almacen_id)}</span>
              </div>
              {movToDelete.referencia && (
                <div className="flex justify-between">
                  <span className="text-[#64748B] dark:text-[#94A3B8]">Referencia:</span>
                  <span className="italic text-[#64748B] dark:text-[#94A3B8] truncate max-w-[200px]">{movToDelete.referencia}</span>
                </div>
              )}
            </div>
